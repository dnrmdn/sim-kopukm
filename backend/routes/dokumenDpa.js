// backend/src/routes/dokumenDpa.js
...
router.get("/", async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      "SELECT id, name, mime, created_at FROM dokumen_dpa ORDER BY created_at DESC"
    );
...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      "INSERT INTO dokumen_dpa (name, mime, data) VALUES (?, ?, ?)",
      [name, file.mimetype, file.buffer]
    );

    // Ambil data terbaru dari DB (termasuk created_at)
    const [rows] = await pool.execute(
      "SELECT id, name, mime, created_at FROM dokumen_dpa WHERE id = ?",
      [result.insertId]
    );
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      "SELECT name, mime, data FROM dokumen_dpa WHERE id = ?",
      [id]
    );
...
router.put("/:id", async (req, res) => {
  try {
    ...
    await pool.execute("UPDATE dokumen_dpa SET name = ? WHERE id = ?", [name, id]);
...
router.delete("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    await pool.execute("DELETE FROM dokumen_dpa WHERE id = ?", [id]);
