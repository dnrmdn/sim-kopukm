...
export default function OverviewDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchKoperasiSummary = async () => {
      ...
      const endpoints = ["/dashboard/koperasi-summary", "/dashboard/koperasi-summary", "/koperasi-summary", "/koperasi-summary"];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-linear-to-br from-blue-50 via-white to-blue-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 px-6 mt-4">
        {/* Card 1: Sekretariat */}
        <div onClick={() => navigate("/sekretariat")} className="cursor-pointer group bg-white border border-gray-100 rounded-2xl shadow-sm p-6 hover:shadow-xl transition-all hover:-translate-y-1">
        ...
        {/* Card 2: Bidang UMKM */}
        <div onClick={() => navigate("/dashboard")} className="cursor-pointer group bg-white border border-gray-100 rounded-2xl shadow-sm p-6 hover:shadow-xl transition-all hover:-translate-y-1">
          <div className="flex justify-center">
        ...
        <div onClick={() => navigate("/koperasi")} className="cursor-pointer group bg-white border border-gray-100 rounded-2xl shadow-sm p-6 hover:shadow-xl transition-all hover:-translate-y-1">
          <div className="flex justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white border border-gray-100 p-5 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">{loadingUmkm ? "Memuat..." : umkmSummary?.total_umkm?.toLocaleString("id-ID")}</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 p-5 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{loadingKoperasi ? "Memuat..." : errorKoperasi ? "—" : (koperasiSummary?.total_aktif ?? 0).toLocaleString("id-ID")}</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 p-5 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{loadingKoperasi ? "Memuat..." : errorKoperasi ? "—" : Number(koperasiSummary?.kelengkapan?.total_tidak_lengkap ?? 0).toLocaleString("id-ID")}</p>
            </div>
          </div>
        </div>
      </div>
