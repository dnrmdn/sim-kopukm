...
export default function LakipPage() {
  ...
  const handlePreview = (file) => {
    setPreviewFile({ name: file.name, mime: detectMime(file), url: `${api.defaults.baseURL}/api/dokumen/lakip/${file.id}` });
    setPreviewOpen(true);
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setUploadingName("");
        const fileInput = document.getElementById("lakip-upload");
  ...
  const StatusBadge = ({ active }) => active ? (
    ...
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-red-100 text-red-400 text-xs font-bold">✗</span>
  );
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-gray-600 text-sm pl-1">Laporan Akuntabilitas Kinerja Instansi — Kelola dokumen dengan mudah</p>
                </div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Cari nama dokumen LAKIP..."
                  value={search}
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-6">
          ...
          <div className="rounded-2xl border border-blue-200/70 bg-linear-to-br from-white/80 to-blue-50/80 backdrop-blur-sm shadow-xl overflow-hidden">
            ...
            <div className="p-6 sm:p-8">
              ...
              <p className="text-gray-500 text-sm mb-6 pl-1">Status kelengkapan dokumen per tahun (Triwulan I–IV & Tahunan)</p>

              <div className="overflow-x-auto rounded-xl border border-blue-200/70">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-linear-to-r from-blue-50 to-cyan-50 border-b border-blue-200/70">
                      <th className="px-5 py-3 text-left font-bold text-gray-800 uppercase tracking-wide text-xs">Tahun</th>
                      <th className="px-5 py-3 text-center font-bold text-gray-800 uppercase tracking-wide text-xs">TW I</th>
                      <th className="px-5 py-3 text-center font-bold text-gray-800 uppercase tracking-wide text-xs">TW II</th>
                      <th className="px-5 py-3 text-center font-bold text-gray-800 uppercase tracking-wide text-xs">TW III</th>
                      <th className="px-5 py-3 text-center font-bold text-gray-800 uppercase tracking-wide text-xs">TW IV</th>
                      <th className="px-5 py-3 text-center font-bold text-gray-800 uppercase tracking-wide text-xs">Tahunan</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rekapLakip.map((r) => (
                      <tr key={r.tahun} className="border-b border-blue-100/70 hover:bg-blue-50/50 transition-all duration-200 bg-white/50">
                        <td className="px-5 py-3 font-semibold text-gray-800">{r.tahun}</td>
                        <td className="px-5 py-3 text-center"><StatusBadge active={r.q1} /></td>
                        <td className="px-5 py-3 text-center"><StatusBadge active={r.q2} /></td>
                        <td className="px-5 py-3 text-center"><StatusBadge active={r.q3} /></td>
                        <td className="px-5 py-3 text-center"><StatusBadge active={r.q4} /></td>
                        <td className="px-5 py-3 text-center"><StatusBadge active={r.tahunan} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="rounded-2xl border border-blue-200/70 bg-linear-to-br from-white/80 to-blue-50/80 backdrop-blur-sm shadow-xl overflow-hidden">
            ...
            <div className="p-6 sm:p-8">
              ...
              <form onSubmit={handleUpload} className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <input
                  type="text"
                  placeholder="Contoh: LAKIP Tahun 2025 Triwulan I"
                  value={uploadingName}
          ...
          <div className="space-y-3">
            ...
            <div className="p-4 rounded-xl border border-blue-200/70 bg-linear-to-br from-white/80 to-blue-50/80 backdrop-blur-sm hover:border-blue-300 transition-all duration-200 w-fit">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-600 uppercase tracking-wide">Total Dokumen</p>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-24">
                ...
                <p className="text-gray-600 font-medium">Memuat dokumen LAKIP...</p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-blue-300 rounded-3xl bg-linear-to-br from-blue-100/30 to-cyan-100/30">
                ...
                <p className="text-xl font-bold text-gray-800 mb-2">
                  {search ? "Tidak ada hasil pencarian" : "Belum ada dokumen LAKIP"}
                </p>
                <p className="text-gray-600 text-sm text-center max-w-sm">
                  {search ? "Coba ubah kata kunci pencarian" : "Upload dokumen LAKIP untuk memulai"}
                </p>
              </div>
            ) : (
              <>
