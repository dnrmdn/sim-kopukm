...
export default function LakipPage() {
  ...
  const handlePreview = (file) => {
    setPreviewFile({
      ...
      url: `${api.defaults.baseURL}/api/dokumen/lakip/${file.id}`,
    });
    setPreviewOpen(true);
  };
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-8 py-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400 text-sm">
                Laporan Akuntabilitas Kinerja Instansi - Kelola dokumen dengan mudah
              </p>
            </div>
          </div>
        </header>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-6">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-white/20 bg-linear-to-br from-slate-900/40 to-slate-800/30 backdrop-blur-xl shadow-xl">
            ...
            <div className="relative p-6 sm:p-8">
              <h2 className="text-lg font-bold text-white mb-2">📊 Rekapitulasi LAKIP</h2>
              <p className="text-slate-400 text-sm mb-6">Cek status kelengkapan dokumen per tahun (Triwulan I–IV & Tahunan)</p>

              <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-900/60 backdrop-blur-sm border-b border-white/10">
                      <th className="px-4 py-3 text-left font-bold text-white">Tahun</th>
                      <th className="px-4 py-3 text-center font-bold text-white">TW I</th>
                      <th className="px-4 py-3 text-center font-bold text-white">TW II</th>
                      <th className="px-4 py-3 text-center font-bold text-white">TW III</th>
                      <th className="px-4 py-3 text-center font-bold text-white">TW IV</th>
                      <th className="px-4 py-3 text-center font-bold text-white">Tahunan</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rekapLakip.map((r) => (
                      ...
                      >
                        <td className="px-4 py-3 font-semibold text-white">{r.tahun}</td>
                        <td className="px-4 py-3 text-center text-lg">
                          {r.q1 ? <span className="text-emerald-400">✅</span> : <span className="text-red-400">❌</span>}
                        </td>
                        <td className="px-4 py-3 text-center text-lg">
                          {r.q2 ? <span className="text-emerald-400">✅</span> : <span className="text-red-400">❌</span>}
                        </td>
                        <td className="px-4 py-3 text-center text-lg">
                          {r.q3 ? <span className="text-emerald-400">✅</span> : <span className="text-red-400">❌</span>}
                        </td>
                        <td className="px-4 py-3 text-center text-lg">
                          {r.q4 ? <span className="text-emerald-400">✅</span> : <span className="text-red-400">❌</span>}
                        </td>
                        <td className="px-4 py-3 text-center text-lg">
                          {r.tahunan ? <span className="text-emerald-400">✅</span> : <span className="text-red-400">❌</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-white/10 rounded-3xl bg-linear-to-br from-slate-800/20 to-slate-700/10">
              ...
              <p className="text-slate-400 text-sm mb-6 text-center max-w-sm">
                {search ? "Coba ubah pencarian" : "Upload dokumen LAKIP pertama Anda sekarang"}
              </p>
            </div>
          ) : (
