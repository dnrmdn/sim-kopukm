...
export default function TabelProgram({ apiBase = "/renstra/program" }) {
  ...
  return (
    <div className="w-full bg-white overflow-hidden flex flex-col h-full border border-slate-200 rounded-lg shadow-sm">
      {/* TOOLBAR REFRESH SAJA */}
      {/* <div className="p-3 bg-slate-50 border-b border-slate-200 flex justify-end items-center">
        <button 
          onClick={fetchData} 
          className="p-2 hover:bg-slate-200 rounded-md transition-all border border-slate-200 bg-white shadow-sm"
          title="Refresh Data"
        >
            ...
            {data.map((p) => (
              <React.Fragment key={`p-${p.id}`}>
                <tr className="bg-blue-50/40 hover:bg-blue-50 transition-colors group text-[13px]">
                  <td className="sticky left-0 z-40 bg-white group-hover:bg-blue-50 px-6 py-5 border-b border-r border-slate-200 font-black text-blue-900 cursor-pointer" onClick={() => toggleProgram(p.id)}>
                  ...
                  <td className="px-4 py-5 border-b border-r border-slate-200 font-bold text-blue-700 italic bg-blue-50/20">{p.indikator_program || "-"}</td>
                  <td className="px-4 py-5 border-b border-r border-slate-200 font-medium text-slate-500 italic">{p.output_program || "-"}</td>
                  ...
                  {YEARS.map(y => {
                    ...
                    return (
                      <React.Fragment key={`p-val-${y}`}>
                        <td className="px-3 py-5 border-b border-r border-slate-200 text-center font-black text-slate-700">{ang ? parseFloat(ang.target).toLocaleString('id-ID') : "0"}</td>
                        <td className="px-3 py-5 border-b border-r border-slate-200 text-right font-black text-blue-700 bg-blue-50/10">{ang ? Number(ang.pagu).toLocaleString('id-ID') : "0"}</td>
                      </React.Fragment>
                    );
                  })}
                  <td className="sticky right-0 z-40 bg-white group-hover:bg-blue-50 px-4 py-5 border-b border-l border-slate-200">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openAddKegiatan(p)} className="p-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all shadow-sm active:scale-90" title="Tambah Kegiatan"><Plus size={14} /></button>
                      <button onClick={() => openEditProgram(p)} className="p-2 bg-amber-500 text-white rounded-md hover:bg-amber-600 transition-all shadow-sm active:scale-90" title="Edit Program"><Edit3 size={14} /></button>
                      <button onClick={() => handleDelete(p)} className="p-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-all shadow-sm active:scale-90" title="Hapus Program"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
                {expandedPrograms[p.id] && (
                  <TabelKegiatan kegiatans={kegiatanData[p.id] || []} YEARS={YEARS} onSuccess={fetchData} />
                )}
              </React.Fragment>
            ))}
          </tbody>
      ...
      <AddKegiatanModal open={isModalKegiatanOpen} onClose={() => setIsModalKegiatanOpen(false)} onSuccess={fetchData} programId={activeProgram?.id} programName={activeProgram?.nama_program} />
