...
export default function SopPage() {
  ...
  const handlePreview = (file) => {
    setPreviewFile({
      ...
      url: `${api.defaults.baseURL}/api/dokumen/lke/${file.id}`,
    });
    setPreviewOpen(true);
  };
  ...
  const handleUpload = async (e) => {
    ...
    if (res.data?.success) {
      ...
      setUploadingName("");
      document.getElementById("lke-upload-input").value = "";
    }
  };
      ...
      <div className="max-w-[1200px] mx-auto space-y-8">
        ...
        <section className="bg-white p-6 rounded-2xl shadow border">
          <h3 className="font-bold flex items-center gap-2 mb-4">
            <UploadCloud className="text-sky-600" /> Upload LKE
          </h3>
        ...
        <section className="bg-white p-6 rounded-2xl shadow border">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            ...
            <div className="text-sm font-semibold text-slate-600 bg-slate-100 px-4 py-2 rounded-full">
              📄 Menampilkan {filteredFiles.length} dari {files.length} LKE
            </div>
          </div>
          ...
          {loading ? (
            ...
            <table className="w-full text-sm border rounded-lg overflow-hidden">
              <thead className="bg-slate-100">
                <tr>
                  <th className="p-3 text-left">Nama LKE</th>
              ...
              <tbody>
                ...
                {filteredFiles.length === 0 && (
                  <tr>
                    <td colSpan={3} className="text-center p-6 text-slate-500">Tidak ada dokumen LKE</td>
                  </tr>
                )}
              </tbody>
            </table>
