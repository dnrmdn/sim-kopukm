...
import { PencilLine, Trash2, Plus, Search, Eye, FileText, AlertCircle, ArrowLeft, UploadCloud, CheckCircle } from "lucide-react";
...
export default function SpipPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    } catch { setError("Gagal memuat data SPIP"); }
  ...
  const handlePreview = (file) => {
    setPreviewFile({ name: file.name, mime: detectMime(file), url: `${api.defaults.baseURL}/api/dokumen/spip/${file.id}` });
    setPreviewOpen(true);
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setUploadingName("");
        const el = document.getElementById("spip-upload-input");
        if (el) el.value = "";
        setUploadSuccess(true);
        setTimeout(() => setUploadSuccess(false), 3000);
      } else { setUploadError(res.data?.message || "Gagal upload"); }
    } catch (err) { setUploadError(err.response?.data?.message || "Terjadi kesalahan saat upload"); }
            ...
            <div className="flex flex-col gap-5">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-gray-600 text-sm pl-1">Sistem Pengendalian Internal Pemerintah — Kelola dokumen dengan mudah</p>
                </div>
                <button
                  onClick={() => navigate(-1)}
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Cari nama dokumen SPIP..."
                  value={search}
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-6">
          ...
          <div className="rounded-2xl border border-blue-200/70 bg-linear-to-br from-white/80 to-blue-50/80 backdrop-blur-sm shadow-xl overflow-hidden">
            ...
            <div className="p-6 sm:p-8">
              <h2 className="font-bold text-gray-800 flex items-center gap-2 mb-5">
                <div className="p-1.5 rounded-lg bg-linear-to-br from-blue-500 to-cyan-400"><UploadCloud size={16} className="text-white" /></div>
                Upload Dokumen SPIP
              </h2>
              ...
              <form onSubmit={handleUpload} className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <label className="relative cursor-pointer md:col-span-1">
                  <input id="spip-upload-input" type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onChange={handleFileChange} className="absolute inset-0 opacity-0 w-full h-full cursor-pointer" />
          ...
          <div className="p-4 rounded-xl border border-blue-200/70 bg-linear-to-br from-white/80 to-blue-50/80 backdrop-blur-sm w-fit">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-600 uppercase tracking-wide">Total Dokumen SPIP</p>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-blue-300 rounded-3xl bg-linear-to-br from-blue-100/30 to-cyan-100/30">
              ...
              <p className="text-xl font-bold text-gray-800 mb-2">{search ? "Tidak ada hasil pencarian" : "Belum ada dokumen SPIP"}</p>
              <p className="text-gray-600 text-sm text-center max-w-sm">{search ? "Coba ubah kata kunci pencarian" : "Upload dokumen SPIP untuk memulai"}</p>
            </div>
          ) : (
            <>
              ...
              <div className="text-xs text-gray-600 text-center">Menampilkan {filteredFiles.length} dari {files.length} dokumen</div>
            </>
          )}
        </main>
